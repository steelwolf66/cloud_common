...
import com.ztax.common.config.PropertyHolder;
...
public class WaterMarkHandler implements SheetWriteHandler {
    ...
    public void afterSheetCreate(WriteWorkbookHolder writeWorkbookHolder, WriteSheetHolder writeSheetHolder) {
        if (PropertyHolder.isEnableExcelWaterMark()) {
            try {
                ByteArrayOutputStream waterMark = createWaterMark(PropertyHolder.getExcelWaterMarkContent());
                XSSFSheet sheet = (XSSFSheet) writeSheetHolder.getSheet();
                putWaterRemarkToExcel(sheet, waterMark.toByteArray());
            }catch (Exception e) {
                logger.error("向Excel文件内添加水印发生异常！", e);
            }
        }
    }
}